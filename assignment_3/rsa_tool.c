...
int main(int argc, char *argv[]){
    ...
    if (mode == 0) {        // key generation
        ...
        */
        ...
        // set an effective outfile (quick-and-dirty way to deal with the case of same infile and outfile, see line 226)
        const char *effective_outfile_name = (strcmp(infile_name, outfile_name) == 0) ? TMP_FILENAME : outfile_name;

        FILE *outfile = fopen(effective_outfile_name, "w+");
        ...
        // handle the case of the effective outfile name (see line 197)
        if (strcmp(effective_outfile_name, outfile_name) != 0) {
            outfile = fopen(effective_outfile_name, "r");
            FILE *actual_outfile  = fopen(outfile_name, "w");
    
            if (actual_outfile == NULL || outfile == NULL) {
                printf("Unable to open file.\n");
                exit(-1);
            }

            // copy file content
            char ch = fgetc(outfile);
            while (ch != EOF) {
                fputc(ch, actual_outfile);
                ch = fgetc(outfile);
            }

            fclose(outfile);
            fclose(actual_outfile);
            remove(TMP_FILENAME);
        }
        ...
        // set an effective outfile (quick-and-dirty way to deal with the case of same infile and outfile, see line 293)
        const char *effective_outfile_name = (strcmp(infile_name, outfile_name) == 0) ? TMP_FILENAME : outfile_name;
        ...
        // handle the case of the effective outfile name (see line 269)
        if (strcmp(effective_outfile_name, outfile_name) != 0) {
            outfile = fopen(effective_outfile_name, "r");
            FILE *actual_outfile  = fopen(outfile_name, "w");
